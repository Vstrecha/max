...
FastAPI приложение для Max Mini App с полной CRUD функциональностью.
...
- **Authorization**: `tma <init_data>` - Max Mini App init data
...
- Все API endpoints (`/v1/*`) требуют Max авторизацию
...
### Тестовые данные
- Используется `create_test_init_data()` для генерации валидных Max init data
...
- Max Mini App авторизация для всех API endpoints
